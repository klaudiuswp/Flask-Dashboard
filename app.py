...
def index():
    ...
    cat_order = df2.groupby('Category').agg({'App' : 'count'}).sort_values('App',ascending=False).reset_index().head(5)
    ...
    # group_game = df2[df2['Category']=='GAME'].groupby('Content Rating').count().reset_index()
    # Width = group_game['App'].values
    # Y = group_game['Content Rating'].values
    # plt.barh(Y,Width)

    most_profit = df2[df2['Type']=='Paid'].groupby('Category').mean()[['Installs','Price']].dropna()
    most_profit['Profit'] = (most_profit['Installs']*most_profit['Price']).astype(int)
    most_profit = most_profit.sort_values('Profit',ascending=False).reset_index().head(5)
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    Width = (most_profit['Profit']/1000000).values
    Y = most_profit['Category'].values
    plt.bar(Y,Width,color='yrbkm')
    plt.xlabel('Category')
